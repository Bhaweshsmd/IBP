...
@section('content')
    <div class="card">
        ...
        <div class="card-body">
            ...
            <form action="{{ route('faqs.update', $faq->id) }}" method="Post" enctype="multipart/form-data" onsubmit='disableformButton()'>
                @csrf
                ...
                <div class="form-group">
                    ...
                    <select name="category_id" id="category" class="form-control form-control-sm" aria-label="Default select example">
                        @foreach ($cats as $cat)
                            <option value="{{ $cat->id }}" @if($faq->category_id == $cat->id) selected @endif>{{ $cat->title }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                @foreach($languages as $language)
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control" name="{{$lang_desc}}" value="{{$faq->$lang_desc}}">
                    </div>
                    
                    <div class="form-group">
                        <label>{{ $language->name }} Answer</label>
                        <textarea class="form-control" name="{{$lang_title}}">{{$faq->$lang_title}}</textarea>
                    </div>
                @endforeach
