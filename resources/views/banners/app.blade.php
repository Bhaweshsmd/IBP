...
@section('content')
    <div class="card mt-3">
        <div class="card-header">
            ...
            @if(has_permission(session()->get('user_type'), 'add_banners'))
                <a data-toggle="modal" data-target="#addBannerModal" href="" class="ml-auto btn btn-primary text-white"><i class="fa fa-plus"></i> {{ __('Add New App Banner') }}</a>
            @endif
        </div>
        <div class="card-body">
        ...
        <div class="modal-dialog modal-dialog-centered" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5>{{ __('Add New App Banner') }}</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
                <div class="modal-body">
                    <form action="" method="post" enctype="multipart/form-data" id="addBannerForm" autocomplete="off" onsubmit='disableformButton()'>
                        @csrf
                        <div class="form-group">
                            <label>{{ __('App Banner') }}</label>
