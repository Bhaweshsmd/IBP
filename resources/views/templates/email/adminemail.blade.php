...
@section('content')
    <div class="card mt-3">
        <div class="card-header">
            <h4>{{ __('Admin Email Templates') }}</h4>
            @if(has_permission(session()->get('user_type'), 'add_admin_emails'))
                <a  href="{{route('admin.email.templates.store')}}" class="ml-auto btn btn-primary text-white"><i class="fa fa-plus"></i> {{ __('Add New Email Templates') }}</a>
            @endif
           </div>
        <div class="card-body">
