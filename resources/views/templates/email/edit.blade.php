...
@section('content')
    <div class="card mt-3">
        <div class="card-header">
            ...
            <a  href="{{route('email.templates')}}" class="ml-auto btn btn-primary text-white"><i class="fa fa-chevron-left"></i> {{ __('Back to Email Templates') }}</a>
           </div>
        <div class="card-body">
                    ...
                    <form  method="post" enctype="multipart/form-data"   autocomplete="off">
                      ...
                      <div class="row">
                        ...
                        <div class="form-group col-sm-4" >
                            ...
                            <input type="text" name="email_subjects" id="email_subjects"  class="form-control" value="{{old('email_subjects',$emailTemplate['email_subjects'])}}" required>
                        </div>
                        <div class="form-group col-sm-4">
                            <label> {{ __(' Subject In Papiamentu') }}</label>
                            <input type="text" name="email_subject_pap" id="email_subject_pap" value="{{ old('email_subject_pap',$emailTemplate['email_subject_pap']) }}" class="form-control" >
                        </div>
                        <div class="form-group col-sm-4">
                            <label> {{ __('Subject In Dutch ') }}</label>
                            <input type="text" name="email_subject_nl"  id ="email_subjects_nl" value="{{ old('email_subject_nl',$emailTemplate['email_subject_nl']) }}" class="form-control"  >
                        </div>
                        <div class="form-group col-sm-12">
                            <label> {{ __('Content In English') }}</label>
                            <textarea type="text" name="email_content"  class="summernote-simple" id="summernote"  required  >{{old('email_content',$emailTemplate['email_content'])}}</textarea>
                        </div>
                         <div class="form-group col-sm-12">
                            <label> {{ __('Content In Papiamentu') }}</label>
                            <textarea type="text" name="email_content_pap"   class="summernote-simple" id="summernote"  >{{old('email_content_pap',$emailTemplate['email_content_pap'])}}</textarea>
                        </div>
                         <div class="form-group col-sm-12">
                            <label> {{ __('Content In Dutch') }}</label>
                            <textarea type="text" name="email_content_nl"    class="summernote-simple" id="summernote"  >{{old('email_content_nl',$emailTemplate['email_content_nl'])}}</textarea>
                        </div>
                        
                        @if(has_permission(session()->get('user_type'), 'edit_emails'))
