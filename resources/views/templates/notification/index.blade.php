...
@section('content')
    <div class="card mt-3">
        <div class="card-header">
            <h4>{{ __('Customer Notification Templates') }}</h4>
            @if(has_permission(session()->get('user_type'), 'add_notification'))
                <a  href="{{route('notification.templates.store')}}" class="ml-auto btn btn-primary text-white"><i class="fa fa-plus"></i> {{ __('Add New Notification Template') }}</a>
            @endif
           </div>
        <div class="card-body">
