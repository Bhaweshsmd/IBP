@extends('include.app')
@section('header')
    <script src="{{ asset('asset/script/adminNotificationContent.js') }}"></script>
@endsection

@section('content')
    <div class="card mt-3">
        <div class="card-header">
            <h4>{{ __('Add New Admin Notification') }}</h4>
            <a  href="{{route('admin.notification.templates')}}" class="ml-auto btn btn-primary text-white"><i class="fa fa-chevron-left"></i> {{ __('Back to Admin Notification Template') }}</a>
        </div>
        <div class="card-body">
                    ...
                    <form  method="post" enctype="multipart/form-data"   autocomplete="off">
                      ...
                      <div class="row">
                        ...
                        <div class="form-group col-sm-4" >
                            ...
                            <input type="text" name="notification_subjects" id="notification_subjects"  class="form-control" value="{{ old('notification_subjects') }}" required>
                        </div>
                        <div class="form-group col-sm-4">
                            <label> {{ __(' Subject In Papiamentu') }}</label>
                            <input type="text" name="notification_subject_pap" id="notification_subject_pap" value="{{ old('notification_subject_pap') }}" class="form-control" >
                        </div>
                        <div class="form-group col-sm-4">
                            <label> {{ __('Subject In Dutch ') }}</label>
                            <input type="text" name="notification_subject_nl"  id ="notification_subject_nl" value="{{ old('notification_subject_nl') }}" class="form-control"  >
                        </div>
                        <div class="form-group col-sm-12">
                            <label> {{ __('Content In English') }}</label>
                            <textarea type="text" name="notification_content"  class="summernote-simple" id="summernote"  required  >{{old('notification_content')}}</textarea>
                        </div>
                         <div class="form-group col-sm-12">
                            <label> {{ __('Content In Papiamentu') }}</label>
                            <textarea type="text" name="notification_content_pap"   class="summernote-simple" id="summernote"  >{{old('notification_content_pap')}}</textarea>
                        </div>
                         <div class="form-group col-sm-12">
                            <label> {{ __('Content In Dutch') }}</label>
                            <textarea type="text" name="notification_content_nl"    class="summernote-simple" id="summernote"  >{{old('notification_content_nl')}}</textarea>
                        </div>
