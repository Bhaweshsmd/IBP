@extends('include.app')
@section('header')
    <script src="{{ asset('asset/script/maintenancefees.js') }}"></script>
@endsection
...
@section('content')
    <div class="card">
        <div class="card-header">
            <h4>{{ __('Membership Card Fees') }}</h4>
            @if(has_permission(session()->get('user_type'), 'add_card_membership_fee'))
        ...
        <div class="modal-dialog modal-dialog-centered" role="document">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <form method="post" enctype="multipart/form-data" action="{{ route('maintenance.card.fees.store') }}" autocomplete="off" onsubmit='disableformButton()'>
                        @csrf
                        
                        <div class="form-group">
                            <label> {{ __('Fee') }}</label>
                            <input id="fee" type="number" name="fee" min="0" step="any" class="form-control" required>
                        </div>
